...
public class SM3Encoder {
    ...
    public static byte[] hash(File file) throws Exception {
        if (file.exists()) {
            ...
            return hash(StreamUtils.readInputStream2ByteArray(inStream));
        } else {
    ...
    public static String hashHex(byte[] b) {
        try {
            return SM3Encoder.byteArrayToHexString(SM3Encoder.hash(b));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
